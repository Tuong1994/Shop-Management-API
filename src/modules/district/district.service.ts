...
export class DistrictService {
  ...
  async getDistricts(query: QueryDto) {
    const { keywords, sortBy, locale, cityCode } = query;
    const districts = await this.prisma.district.findMany({
      where: { AND: [{ cityCode: cityCode && Number(cityCode) }, { isDelete: { equals: false } }] },
      orderBy: [{ updatedAt: utils.getSortBy(sortBy) ?? 'desc' }],
      select: { ...this.districtHelper.getSelectFields(locale) },
    });
    ...
    const items = this.districtHelper.convertCollection(keywords ? filterDistricts : districts, locale);
    return { totalItems: keywords ? filterDistricts.length : districts.length, items };
  }

  async getDistrictsPaging(query: QueryDto) {
    const { page, limit, keywords, sortBy, locale, cityCode } = query;
    ...
    const items = this.districtHelper.convertCollection(collection.items, locale);
    return { ...collection, items };
  }

  async getDistrict(query: QueryDto) {
    const { districtId, locale } = query;
    const district = await this.prisma.district.findUnique({
      where: { id: districtId, isDelete: { equals: false } },
      select: { ...this.districtHelper.getSelectFields(locale) },
    });
    return utils.convertRecordsName<District>(district, locale);
  }
