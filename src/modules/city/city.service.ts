...
export class CityService {
  ...
  async getCities(query: QueryDto) {
    ...
    const cities = await this.prisma.city.findMany({
      ...
      select: { ...this.cityHelper.getSelectFields(locale) },
    });
    ...
    const items = this.cityHelper.convertCollection(keywords ? filterCities : cities, locale);
  ...
  async getCitiesPaging(query: QueryDto) {
    ...
    const items = this.cityHelper.convertCollection(collection.items, locale);
    return { ...collection, items };
  }

  async getCity(query: QueryDto) {
    const { cityId, locale } = query;
    const city = await this.prisma.city.findUnique({
      where: { id: cityId, isDelete: { equals: false } },
      select: { ...this.cityHelper.getSelectFields(locale) },
    });
    return utils.convertRecordsName<City>(city, locale);
  }
