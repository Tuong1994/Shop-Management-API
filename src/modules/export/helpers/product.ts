...
export const getInventoryStatus = (status: EInventoryStatus, locale: ELang) => {
...
export const getProductUnit = (unit: EProductUnit, locale: ELang) => {
  const lang = utils.getLang(locale);
...
export const getProductOrigin = (origin: EProductOrigin, locale: ELang) => {
  const lang = utils.getLang(locale);
