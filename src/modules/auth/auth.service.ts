...
export class AuthService {
  ...
  async signUp(auth: AuthDto) {
    ...
    const newAccount = await this.prisma.user.create({
      data: { email, password: hashPass, phone, isDelete: false, role: ERole.STAFF },
    });
    return newAccount;
  }
  ...
  async signIn(res: Response, query: QueryDto, auth: AuthDto) {
    ...
    // if (admin && login.role === ERole.STAFF) throw new UnauthorizedException(NOT_AUTHORIZED);
    const info = { ...login };
  ...
  async googleCallback(req: Request, res: Response) {
    ...
    if (!user) {
      const newUser = await this.prisma.user.create({
        data: { fullName: name, email, googleId, password: '', phone: '', role: ERole.STAFF, isDelete: false },
      });
  ...
  async forgotPassword(query: QueryDto, data: AuthForgotPasswordDto) {
    ...
    const subject = locale === ELang.EN ? 'Reset password' : 'Đặt lại mật khẩu';
    try {
      await this.emailHelper.sendGmail({
        to: email,
        subject,
        html: getEmailResetPasswordTemplate(locale, auth.fullName, resetUrl),
      });
