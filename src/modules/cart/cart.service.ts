...
const { UPDATE_SUCCESS, REMOVE_SUCCESS } = responseMessage;
...
export class CartService {
  ...
  async getCarts(query: QueryDto) {
    ...
    const carts = await this.prisma.cart.findMany({
      ...
      include: { items: { select: { ...this.cartHelper.getSelectFields(locale) } } },
    });
    const convertCarts = carts.map((cart) => ({
      ...cart,
      items: cart.items.map((item) => ({
        ...item,
        product: { ...utils.convertRecordsName(item.product, locale) },
      })),
    }));
  ...
  async getCart(query: QueryDto) {
    const { page, limit, userId, locale } = query;
        ...
        include: {
          items: {
            select: {
              quantity: true,
              product: {
                select: { id: true, nameEn: true, nameVn: true, image: true, price: true },
              },
            },
          },
        },
