...
export class UserService {
  ...
  async getUsers(query: QueryDto) {
    const { page, limit, locale, keywords, sortBy, gender, role } = query;
    let collection: Paging<UserResponse> = utils.defaultCollection();
    const users = await this.prisma.user.findMany({
      where: {
        AND: [{ gender: gender && Number(gender) }, { role: role && Number(role) }, { isDelete: { equals: false } }],
      },
      select: {
        ...this.userHelper.getSelectFields(),
        address: { select: { ...this.userHelper.getSelectAddressFields(locale) } },
    ...
    const items = this.userHelper.convertCollection(collection.items, locale);
    return { ...collection, items };
  }

  async getUser(query: QueryDto) {
    const { userId, locale } = query;
    ...
    return {
      ...
      address: user.address
        ? {
            ...
            ...this.userHelper.convertAddress(user.address, locale),
          }
        : null,
    };
  }
  ...
  async createUser(query: QueryDto, file: Express.Multer.File, user: UserDto) {
    ...
    const fullName = this.userHelper.getFullName(firstName, lastName, locale);
  ...
  async removeUsersPermanent(query: QueryDto) {
    ...
    await Promise.all(
      users.map(async (user) => {
        if (user.image) await this.cloudinary.destroy(user.image.publicId);
      }),
    );
