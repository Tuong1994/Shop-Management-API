...
export class UserHelper {
  ...
  getFullName(firstName: string, lastName: string, locale: ELang) {
    if (!firstName && !lastName) return '';
    if (!firstName && lastName) return '';
    if (firstName && !lastName) return '';
    if (locale === ELang.EN) return `${lastName} ${firstName}`;
  ...
  async getFullAddress(
    ...
    const isEnglish = locale === ELang.EN;
  ...
  convertAddress<M>(record: M, locale: ELang) {
    ...
    const data = {
      address: locale === ELang.EN ? recordClone['addressEn'] : recordClone['addressVn'],
      fullAddress: locale === ELang.EN ? recordClone['fullAddressEn'] : recordClone['fullAddressVn'],
      ...record,
    };
    return data;
  }
  ...
  getSelectAddressFields(locale: ELang): Prisma.UserAddressSelect {
    return {
      id: true,
      addressEn: true,
      addressVn: true,
      fullAddressEn: locale === ELang.EN,
      fullAddressVn: locale === ELang.VN,
      cityCode: true,
      districtCode: true,
      wardCode: true,
      userId: true,
  ...
  convertCollection(users: UserResponse[], locale: ELang) {
    return users.map((user) => ({
      ...user,
      address:
        'address' in user ? this.convertAddress<UserAddress>(user.address as UserAddress, locale) : null,
    }));
  }
