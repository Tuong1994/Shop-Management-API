...
export class WardService {
  ...
  async getWards(query: QueryDto) {
    const { keywords, sortBy, locale, districtCode } = query;
    const wards = await this.prisma.ward.findMany({
      where: {
        AND: [{ districtCode: districtCode && Number(districtCode) }, { isDelete: { equals: false } }],
      },
      orderBy: [{ updatedAt: utils.getSortBy(sortBy) ?? 'desc' }],
      select: { ...this.wardHelper.getSelectFields(locale) },
    });
    ...
    const items = this.wardHelper.convertCollection(keywords ? filterWards : wards, locale);
    return { totalItems: keywords ? filterWards.length : wards.length, items };
  }

  async getWardsPaging(query: QueryDto) {
    const { page, limit, keywords, sortBy, locale, districtCode } = query;
    ...
    const items = this.wardHelper.convertCollection(collection.items, locale);
    return { ...collection, items };
  }

  async getWard(query: QueryDto) {
    const { wardId, locale } = query;
    const ward = await this.prisma.ward.findUnique({
      where: { id: wardId, isDelete: { equals: false } },
      select: { ...this.wardHelper.getSelectFields(locale) },
    });
    return utils.convertRecordsName<Ward>(ward, locale);
  }
